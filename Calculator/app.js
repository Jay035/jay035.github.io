...
const checkThree = document.getElementById('toggleThree');
 ...
 document.documentElement.setAttribute('data-theme', 'blue') 
  ...
  if(this.checked){
    trans(); document.documentElement.setAttribute('data-theme', 'milky') 
   } })
 
 checkThree.addEventListener('change', function() { 
 if(this.checked){
  trans(); document.documentElement.setAttribute('data-theme', 'purple') } })
  let trans = () => { 
  ...
  document.documentElement.classList.remove('transition'); },1000) }
   ...
   else if(e.target.innerText === '.' && dot){ return; } 
   displayTwo += e.target.innerText; secondDisplay.innerText = displayTwo; })})
    // <<<<<<< HEAD // =======
    operations.forEach(operation =>{ operation.addEventListener('click', (e)=>{ 
    if(!displayTwo) result; dot = false; const operationName = e.target.innerText; if(displayOne && displayTwo && lastOperation){
     mathOperation()
      } else{ 
 ...
 function clearVal(){ 
       secondDisplay.innerText = ' ' 
       displayTwo = ' '
        console.log(result); temporaryDisplay.innerText = result;
        } 
   ...
   function mathOperation(){ 
        if(lastOperation === 'x'){
         result = parseFloat(result) * parseFloat(displayTwo); } 
          ...
          result = parseFloat(result) + parseFloat(displayTwo); } else if (lastOperation === '-'){
           result = parseFloat(result) - parseFloat(displayTwo); } else if (lastOperation === '/'){
            result = parseFloat(result) / parseFloat(displayTwo); }} equal.addEventListener('click', 
            			(e)=>{ if(!displayOne || !displayTwo) return; mathOperation(); clearVal(); secondDisplay.innerText = result; temporaryDisplay.innerText = '' 
          			displayTwo = result; firstDisplay.innerText = ''; 
          			displayOne = ''
            			 })
          ...
          temporaryDisplay.innerText = '0';
           firstDisplay.innerText = '0';
            displayOne = ''}) // if(window.matchMedia('(prefers-color-scheme)').media !== 'not all'){// console.log('Dark mode is supported');// }
